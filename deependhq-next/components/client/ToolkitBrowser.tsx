"use client";
// ToolkitBrowser : the /toolkit grid with category filter and search, port of
// ToolkitPage.jsx. Receives the toolkit items as serializable props from the
// server page; never imports lib/data.
...
const CATEGORIES: { id: string; label: string; color: string | null }[] = [
...
const CAT_COLOR: Record<string, string> = { tool: "green", repo: "blue", skill: "gold", resource: "muted" };
const CAT_LABEL: Record<string, string> = { tool: "Tool", repo: "Repo", skill: "Skill", resource: "Resource" };

export function ToolkitBrowser({ items }: { items: ToolkitItem[] }) {
  ...
  const counts = items.reduce<Record<string, number>>((a, i) => { a[i.category] = (a[i.category] || 0) + 1; return a; }, {});
  ...
  return (
    <div className="dh-page">
      ...
      <aside className="dh-rail" aria-label="toolkit context">
        <RailProgress label="Drawer position" />
